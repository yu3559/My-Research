...
def load_manual_text(manual_path: str | Path | None, max_chars: int = 4000) -> str:
...
def get_text_embedding(text: str, model_name: str) -> np.ndarray:
    if model_name not in _TEXT_EMBEDDER_CACHE:
        try:
            from sentence_transformers import SentenceTransformer
        except Exception as e:
            raise RuntimeError(
                "text latent を使うには sentence-transformers が必要です。"
                "pip install sentence-transformers を実行してください。"
            ) from e
        _TEXT_EMBEDDER_CACHE[model_name] = SentenceTransformer(model_name)
    model = _TEXT_EMBEDDER_CACHE[model_name]
    prefixed = text if text.startswith("passage:") else f"passage: {text}"
    emb = model.encode(prefixed, normalize_embeddings=True, convert_to_numpy=True, show_progress_bar=False)
    return np.asarray(emb, dtype=np.float32)


def build_context_features_from_aabbs(aabbs: list[dict], n_parts: int) -> np.ndarray | None:
    if n_parts <= 0 or not aabbs:
        return None
    centers = np.zeros((n_parts, 3), dtype=np.float32)
    sizes = np.zeros((n_parts, 3), dtype=np.float32)
    valid = np.zeros((n_parts,), dtype=bool)
    ...
    center_norm = (centers - center_mean) / center_std

    size_log = np.log(np.maximum(sizes, 1e-6))
    size_norm = (size_log - size_log.mean(axis=0, keepdims=True)) / (size_log.std(axis=0, keepdims=True) + 1e-6)

    volume = np.prod(sizes, axis=1, keepdims=True)
    volume_log = np.log(np.maximum(volume, 1e-6))
    volume_norm = (volume_log - volume_log.mean(axis=0, keepdims=True)) / (volume_log.std(axis=0, keepdims=True) + 1e-6)

    y_rank = np.empty((n_parts,), dtype=np.float32)
    y_order = np.argsort(centers[:, 1])
    y_rank[y_order] = np.arange(n_parts, dtype=np.float32)
    y_rank = y_rank / max(float(n_parts - 1), 1.0)
    y_rank = y_rank.reshape(-1, 1)
    ...
    if n_parts > 1:
        ...
        knn_mean = knn.mean(axis=1, keepdims=True)
        knn_min = knn[:, :1]
        knn_mean = (knn_mean - knn_mean.mean(axis=0, keepdims=True)) / (knn_mean.std(axis=0, keepdims=True) + 1e-6)
        knn_min = (knn_min - knn_min.mean(axis=0, keepdims=True)) / (knn_min.std(axis=0, keepdims=True) + 1e-6)
        near_thr = float(np.median(knn[:, 0])) * 1.25
        near_thr = max(near_thr, 1e-6)
        neigh_count = (dist < near_thr).sum(axis=1, keepdims=True).astype(np.float32)
        neigh_ratio = neigh_count / max(float(n_parts - 1), 1.0)
        unit = diff / (np.linalg.norm(diff, axis=2, keepdims=True) + 1e-6)
        unit[np.arange(n_parts), np.arange(n_parts), :] = 0.0
        mean_dir = unit.mean(axis=1)
    else:
        knn_mean = np.zeros((n_parts, 1), dtype=np.float32)
        knn_min = np.zeros((n_parts, 1), dtype=np.float32)
        neigh_ratio = np.zeros((n_parts, 1), dtype=np.float32)
        mean_dir = np.zeros((n_parts, 3), dtype=np.float32)

    feats = np.concatenate(
        [center_norm, size_norm, volume_norm, y_rank, knn_mean, knn_min, neigh_ratio, mean_dir],
        axis=1,
    )
    return np.asarray(feats, dtype=np.float32)


class TextLatentHeads(nn.Module):
    def __init__(self, text_dim: int, part_dim: int, target_dim: int, latent_dim: int = 32, hidden_dim: int = 128):
        super().__init__()
        ...
        self.part_net = nn.Sequential(
            nn.LayerNorm(part_dim + latent_dim),
            nn.Linear(part_dim + latent_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, target_dim),
        )

    def forward(self, text_emb: torch.Tensor, part_vecs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if text_emb.dim() == 1:
            text_emb = text_emb.unsqueeze(0)
        r = self.text_net(text_emb).squeeze(0)
        r = torch.nn.functional.layer_norm(r, r.shape)
        r_expand = r.unsqueeze(0).expand(part_vecs.shape[0], -1)
        p = self.part_net(torch.cat([part_vecs, r_expand], dim=1))
        p = torch.nn.functional.normalize(p, p=2, dim=1)
        return r, p


def compute_text_latent_energy(
    vecs0: torch.Tensor,
    base1: Path,
    n_candidates: int,
    ...
    if not ckpt_path:
        info["reason"] = "text_head_ckpt が未指定"
        return None, info
    ckpt = Path(ckpt_path)
    if not ckpt.exists():
        info["reason"] = f"ckpt not found: {ckpt}"
        return None, info

    manual_text = load_manual_text(manual_path, max_chars=max_chars)
    if not manual_text:
        info["reason"] = "manual text が空"
        return None, info

    if vecs0.dim() != 2:
        info["reason"] = f"vecs0 shape 不正: {tuple(vecs0.shape)}"
        return None, info
    part_dim = int(vecs0.shape[1])

    aabbs = load_aabbs(base1)
    g_np = build_context_features_from_aabbs(aabbs, n_candidates)
    if g_np is None:
        info["reason"] = "AABB から g_j を構築できない"
        return None, info

    text_emb = get_text_embedding(manual_text, model_name=model_name)
    text_dim = int(text_emb.shape[0])
    target_dim = int(g_np.shape[1])
    ...
    if isinstance(payload, dict) and "state_dict" in payload:
        ...
        info["reason"] = "ckpt の形式が不正"
        return None, info

    head = TextLatentHeads(
        text_dim=text_dim,
        part_dim=part_dim,
    ...
    try:
        ...
        info["reason"] = f"state_dict 読み込み失敗: {e}"
        return None, info
    head.eval()
    ...
    with torch.no_grad():
        ...
        if g.dim() != 2:
            info["reason"] = "g_j shape 不正"
            return None, info
        if g.shape[0] != n_candidates:
            info["reason"] = f"g_j 数が候補数と不一致: {g.shape[0]} vs {n_candidates}"
            return None, info
        g = torch.nn.functional.normalize(g, p=2, dim=1)
        r, p = head(t, part)
        txt_sim = torch.matmul(p, g.t()).cpu().numpy()
        info["enabled"] = True
        info["reason"] = "ok"
        info["text_dim"] = text_dim
        info["g_dim"] = target_dim
        info["r_norm"] = float(torch.norm(r).item())
    return -txt_sim.astype(np.float32), info
...
def energy_search(
    ...
    N0, N1 = sim_mat.shape
    text_meta: dict[str, Any] = {"enabled": False, "reason": "disabled"}
    e_txt_mat_np: np.ndarray | None = None
    if use_text_latent and text_weight != 0.0:
        e_txt_mat_np, text_meta = compute_text_latent_energy(
            vecs0=vecs0,
            base1=base1,
            n_candidates=N1,
            manual_path=manual_path,
            ckpt_path=text_head_ckpt,
            model_name=text_model,
            latent_dim=text_latent_dim,
            hidden_dim=text_hidden_dim,
            max_chars=text_max_chars,
        )
        if text_meta.get("enabled", False):
            print(
                "text latent enabled:"
                f" model={text_model}, latent_dim={text_meta.get('latent_dim')},"
                f" g_dim={text_meta.get('g_dim')}, r_norm={text_meta.get('r_norm', 0.0):.3f}"
            )
        else:
            print(f"text latent disabled: {text_meta.get('reason', 'unknown')}")
    ...
    def try_scip_lp():
        ...
        E_txt = [[0.0 for _ in range(N1)] for _ in range(N0)]
        if e_txt_mat_np is not None and e_txt_mat_np.shape == (N0, N1):
            for i in range(N0):
                for j in range(N1):
                    E_txt[i][j] = float(e_txt_mat_np[i, j])
        ...
        for i in range(N0):
            for j in range(N1):
                ...
                if text_weight != 0.0 and E_txt[i][j] != 0.0:
                    obj_terms.append(float(text_weight) * E_txt[i][j] * x[i][j])
    ...
    if lp_result is not None:
        ...
        if e_txt_mat is not None and text_weight != 0.0:
            for i, j in pairs:
                if i < len(e_txt_mat) and j < len(e_txt_mat[i]):
                    e_txt_sum += float(text_weight) * float(e_txt_mat[i][j])
        e_total_str = f"{e_app_sum:.2f}+{e_geo_sum:.2f}+{e_txt_sum:.2f}"
        print(f"E_total (E^app+E^geo+E^txt): {e_total_str}")
        ...
        if e_app_mat is not None and e_geo_mat is not None:
            with open(e_total_path, "w") as f:
                ...
                for i in range(N0):
                    row_vals = " ".join(
                        (
                            f"{(float(alpha) * float(e_app_mat[i][j])):.2f}"
                            f"+{(float(beta_use) * float(e_geo_mat[i][j])):.2f}"
                            f"+{(float(text_weight) * float(e_txt_mat[i][j])):.2f}"
                            if e_txt_mat is not None
                            else f"{(float(alpha) * float(e_app_mat[i][j])):.2f}+{(float(beta_use) * float(e_geo_mat[i][j])):.2f}"
                        )
                        for j in range(N1)
                    )
                    f.write(f"パーツ {i}: {row_vals}\n")
...
def main():
    ...
    parser.add_argument(
        "--use_text_latent",
        action="store_true",
        help="設計図テキストを埋め込み、潜在関係ベクトル経由で E_ij に反映する。",
    )
    parser.add_argument(
        "--text_weight",
        type=float,
        default=0.0,
        help="テキスト潜在項 E_txt の重み（0 で無効）。",
    )
    parser.add_argument(
        "--text_model",
        default="intfloat/multilingual-e5-small",
        help="sentence-transformers のモデル名（例: intfloat/multilingual-e5-small）。",
    )
    parser.add_argument(
        "--text_head_ckpt",
        default="",
        help="TextLatentHeads の学習済み重み (.pt/.pth)。",
    )
    parser.add_argument(
        "--text_latent_dim",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="テキストMLP/パーツMLPの中間次元。",
    )
    parser.add_argument(
        "--text_max_chars",
        type=int,
        default=4000,
        help="manual テキスト埋め込み時に使う最大文字数。",
    )
    ...
    if args.beta != 0.0 or (args.use_text_latent and args.text_weight != 0.0):
        try:
    ...
    result = energy_search(
        ...
        alpha=args.alpha,
        beta=args.beta,
        use_text_latent=args.use_text_latent,
        text_weight=args.text_weight,
        text_model=args.text_model,
        text_head_ckpt=args.text_head_ckpt,
